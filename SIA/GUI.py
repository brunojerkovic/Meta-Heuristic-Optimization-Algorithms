...
import os
from SIA.SimpleImmunologicalAlgorithm import SimpleImmunologicalAlgorithm
from AlgorithmGUI import AlgorithmGUI
from SIA.Operators.Hypermutation import Hypermutation
...
class GUI(AlgorithmGUI):
    def __init__(self, problem, master_frame, filename, position=(0,0)):
        ...
        tk.Label(self.hyperparameters_frame, fg='black', text='Duplication Number: ').grid(row=4, column=0)
        self.dup_num_entry = tk.Entry(self.hyperparameters_frame, fg='black', bg='white', width=10)
        self.dup_num_entry.insert(tk.END, '3')
        self.dup_num_entry.grid(row=4, column=1)
        # endregion
    ...
    def run_algorithm(self, save=True):
        ...
        sia = SimpleImmunologicalAlgorithm(hypermutation_fn=hypermutation_fn, hypermutation_constant=hypermutation_constant, pop_size=pop_size, iter_num=iter_num, dup_num=dup_num)
        solutions = sia.solve(self.problem)
